...
public partial class OrderPage : ContentPage
{
    ...
    private async void OnCompleteOrderClicked(object sender, EventArgs e)
    {
        if (!_orderItems.Any())
        {
            await DisplayAlert("Error", "Molimo prvo dodajte stavke u narudžbu", "OK");
            return;
        }

        bool confirm = await DisplayAlert("Potvrdi narudžbu",
            $"Ukupna cijena: ${_totalAmount:F2}\nŽelite li završiti narudžbu",
            "Da", "Ne");
        ...
        if (confirm)
        {
            try
            {
                ...
                await DisplayAlert("Uspjeh", "Narudžba uspješno kreirana", "OK");
